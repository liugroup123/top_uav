# -*- coding: utf-8 -*-
"""
使用简化环境的MATD3训练脚本 - 带LSTM增强的GAT模型版本
基于main_clean_env_v3.py，适配带LSTM的GAT模型
"""
...
uav_env_path = os.path.join(parent_dir, 'uav_top_env', 'uav_env_clean_v5_lstm.py')
spec = importlib.util.spec_from_file_location("uav_env_clean_v5_lstm", uav_env_path)
...
# 获取当前文件目录路径 - 更改输出路径包含lstm标识
model_dir = os.path.join(current_dir, './output_clean_env/models/lstm_v5')  # 模型保存文件夹
video_dir = os.path.join(current_dir, './output_clean_env/videos/lstm_v5')  # 视频保存文件夹
runs_dir = os.path.join(current_dir, './output_clean_env/runs/lstm_v5')  # TensorBoard 日志文件
...
def main():
    """主训练函数"""
    print("🚀 开始使用带LSTM的GAT简化环境训练MATD3")
    ...
    for episode in tqdm(range(num_episodes), desc="训练进度"):
        ...
        # 计算并记录覆盖率
        final_coverage_rate, is_fully_connected, episode_max_coverage, unconnected_uav, priority_coverage_rate = env.calculate_coverage_complete()
        ...
        print(f"Episode {episode:4d}: 类型={episode_type:8s} | "
              ...
              f"优先覆盖率={priority_coverage_rate:.3f} | "
        ...
        writer.add_scalar('Performance/Priority_Coverage_Rate', priority_coverage_rate, episode)  # 优先覆盖率
        ...
        if episode % 500 == 0:
            # 修改保存文件名，包含lstm标识
            model_save_path = f"{model_dir}/matd3_lstm_episode_{episode}.pth"
            gat_save_path = f"{model_dir}/gat_lstm_episode_{episode}.pth"

            matd3.save(model_save_path)
            env.save_gat_model(gat_save_path)  # 保存带LSTM的GAT模型

            print(f"💾 MATD3模型已保存: {model_save_path}")
            print(f"🧠 带LSTM的GAT模型已保存: {gat_save_path}")
    ...
    # 保存最终模型
    final_model_path = f"{model_dir}/matd3_lstm_final.pth"
    final_gat_path = f"{model_dir}/gat_lstm_final.pth"
    ...
    print(f"🧠 带LSTM的GAT模型保存至: {final_gat_path}")
