...
uav_env_path = os.path.join(parent_dir, 'uav_top_env', 'uav_env_clean_lstm.py')
spec = importlib.util.spec_from_file_location("uav_env_clean_lstm", uav_env_path)
uav_env_module = importlib.util.module_from_spec(spec)
spec.loader.exec_module(uav_env_module)
UAVEnvLSTM = uav_env_module.UAVEnvLSTM
...
def main():
    ...
    for episode in tqdm(range(num_episodes), desc="训练进度"):
        ...
        if episode % 500 == 0:
            ...
            print(f"🧠 LSTM-GAT模型已保存: {gat_save_path}")
            print(f"   ├─ 双GAT架构 (输出{env.gat_output_dim}维)")

            if env.enable_temporal:
                print(f"   ├─ 时序LSTM ({env.temporal_steps}步)")
            else:
                print(f"   ├─ 时序LSTM: 未启用")
    ...
    print(f"🧠 LSTM-GAT模型保存至: {final_gat_path}")
    print(f"   ├─ 双GAT架构 + 时序LSTM")
    print(f"   └─ 支持{env.temporal_steps}步历史建模")
