"""
LSTM增强的拓扑UAV环境 - 在原有基础上集成时序建模
基于uav_env_clean.py，添加LSTM时序建模能力
"""
...
def _import_lstm_gat_model():
    """动态导入LSTM增强GAT模型"""
    current_dir = os.path.dirname(os.path.abspath(__file__))
    gat_lstm_path = os.path.join(current_dir, 'gat_model_lstm.py')
    ...
    return gat_lstm_module.UAVAttentionNetworkLSTM, gat_lstm_module.create_adjacency_matrices

# 导入两种GAT模型
UAVAttentionNetwork, create_adjacency_matrices = _import_gat_model()
UAVAttentionNetworkLSTM, create_adjacency_matrices_lstm = _import_lstm_gat_model()

def _import_obs_attention():
    """动态导入观察attention模型"""
    current_dir = os.path.dirname(os.path.abspath(__file__))
    obs_attention_path = os.path.join(current_dir, 'uav_obs_attention.py')

    spec = importlib.util.spec_from_file_location("uav_obs_attention", obs_attention_path)
    obs_attention_module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(obs_attention_module)

    return obs_attention_module.UAVObservationAttention, obs_attention_module.create_communication_mask

UAVObservationAttention, create_communication_mask = _import_obs_attention()

# 简化版本不需要复杂的配置导入


class UAVEnv(gym.Env):
    """LSTM增强的拓扑UAV环境 - 在原有基础上添加时序建模"""
    ...
    def __init__(self,
                 ...
                 gat_output_dim=32,
                 # 观察Attention配置
                 use_obs_attention=True,    #是否启用观察Attention
                 ...
                 max_neighbors=3,
                 # LSTM时序配置 (新增)
                 enable_temporal=True,     #是否启用lstm
                 temporal_steps=5):     #5步历史
    ...
    def reset(self, seed=None):
        ...
        # 重置LSTM时序状态 (新增)
        if self.enable_temporal and hasattr(self.gat_model, 'reset_temporal_state'):
    ...
    def _get_obs(self):
        """获取观察 - 集成观察attention的版本"""
        # 1. 计算GAT特征（使用缓存）
        gat_features = self._compute_gat_features()

        # 2. 生成原始观察
        raw_observations = self._compute_raw_observations(gat_features)

        # 3. 应用观察attention增强
        enhanced_observations = self._apply_observation_attention(raw_observations)

        return enhanced_observations

    def _compute_raw_observations(self, gat_features):
        """计算原始观察（不含attention增强）"""
        # 预计算公共数据，避免重复计算
    ...
    def _apply_observation_attention(self, raw_observations):
        ...
        # 创建通信掩码
        agent_positions = torch.FloatTensor(self.agent_pos).to(self.device)
        comm_mask = create_communication_mask(
            agent_positions,
            self.active_agents,
        ...
        with torch.set_grad_enabled(self.training):
            enhanced_obs, attention_weights = self.obs_attention_net(
                obs_tensor,
                comm_mask,
                self.active_agents
            )
    ...
    def save_gat_model(self, path):
        ...
        if self.use_obs_attention and self.obs_attention_net is not None:
            ...
            # 根据LSTM配置决定架构标识
            if self.enable_temporal:
                model_data['architecture'] = 'dual_gat_v5_with_lstm_and_obs_attention'
            else:
            ...
            # 根据LSTM配置决定架构标识
            if self.enable_temporal:
                model_data['architecture'] = 'dual_gat_v5_with_lstm_temporal'
            else:
    ...
    def load_gat_model(self, path):
        ...
        try:
            ...
            if isinstance(model_data, dict) and 'architecture' in model_data:
                ...
                # LSTM版本模型
                if 'lstm' in model_data['architecture']:
                    # LSTM增强版本
                    self.gat_model.load_state_dict(model_data['gat_state_dict'])

                    if model_data['architecture'] == 'dual_gat_v5_with_lstm_and_obs_attention':
                        # LSTM + 观察Attention
                        if self.use_obs_attention and self.obs_attention_net is not None:
                            self.obs_attention_net.load_state_dict(model_data['obs_attention_state_dict'])
                            print(f"✅ 加载完整LSTM增强模型: {model_data['architecture']}")
                            print("✅ LSTM-GAT模型和观察Attention模型均已加载")
                        else:
                            print(f"✅ 加载LSTM-GAT模型: {model_data['architecture']}")
                            print("ℹ️  当前环境未启用观察Attention，只加载了LSTM-GAT部分")

                    elif model_data['architecture'] == 'dual_gat_v5_with_lstm_temporal':
                        # 只有LSTM，没有观察attention
                        print(f"✅ 加载LSTM-GAT模型: {model_data['architecture']}")
                        print("✅ LSTM时序建模已启用")

                    # 验证LSTM配置
                    if saved_config.get('enable_temporal', True) != self.enable_temporal:
                        print(f"⚠️  模型时序配置({saved_config.get('enable_temporal')}) 与当前环境({self.enable_temporal})不匹配")

                # 旧版本GAT模型
                elif model_data['architecture'] == 'dual_gat_v4_with_obs_attention':
                    # 旧版本，包含观察attention但无LSTM
                    if self.enable_temporal:
                        print("⚠️  加载的模型不包含LSTM，LSTM部分将使用随机初始化")
                        self._load_gat_only(model_data['gat_state_dict'])
                    else:
                        self.gat_model.load_state_dict(model_data['gat_state_dict'])
    ...
    def _load_gat_only(self, gat_state_dict):
        """只加载GAT部分，跳过LSTM部分（用于向后兼容）"""
        current_state = self.gat_model.state_dict()

        # 只加载GAT相关的参数，跳过LSTM相关参数
        gat_keys = [k for k in gat_state_dict.keys() if 'temporal' not in k]

        for key in gat_keys:
            if key in current_state:
                current_state[key] = gat_state_dict[key]

        self.gat_model.load_state_dict(current_state)
        print("✅ 已加载GAT部分，LSTM部分保持随机初始化")
