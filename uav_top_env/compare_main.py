...
current_dir = os.path.dirname(os.path.abspath(__file__))
model_dir = os.path.join(current_dir, './output/models_/2月20日——第二版本_2')  # 模型保存文件夹
video_dir = os.path.join(current_dir, './output/videos/2月20日——第二版本_2')  # 视频保存文件夹

import torch
import numpy as np
from pettingzoo.mpe import simple_uav_v1
from matd3_2 import MATD3, ReplayBuffer  # 修改为 MATD3
...
FRAME_SAVE_INTERVAL = 50  # 每100轮保存视频
final_times = 50
...
def main():
    writer = SummaryWriter(log_dir='./runs/2月20 日——第二版本_2')
    ...
    # 初始化环境并设置为 human 模式
    env = simple_uav_v1.parallel_env(render_mode=render_mode)  # 设置为 human 模式
    ...
    if os.path.exists(model_path):
        ...
        # 加载 ReplayBuffer 状态
        replay_buffer = checkpoint.get('replay_buffer', ReplayBuffer(
        ...
        # 如果没有保存的模型，初始化模型到设备
        replay_buffer = ReplayBuffer(buffer_size=CONFIG["buffer_size"], batch_size=CONFIG["batch_size"], device=device)
    ...
    for episode in tqdm(range(num_episodes), desc="Training Progress"):
        ...
        for step in range(max_steps_per_episode):
            ...
            for agent in agents:
                ...
                # 优化数据转换
                observation_tensor = torch.from_numpy(observation).float().unsqueeze(0).to(device, non_blocking=True)
                with torch.no_grad():
                    action_tensor = maddpg.actors[agent](observation_tensor)
                action = action_tensor.cpu().numpy().squeeze(0)
                action += noise[agent].noise()
                actions[agent] = np.clip(action, 0, 1)
            ...
            if record_video and step :  # 每2步保存一次帧，减少渲染频率
                frame = env.render()
        ...
        # 计算并记录覆盖率
        coverage_rate, is_fully_connected, max_coverage_rate,unconnected_uav = env.unwrapped.calculate_coverage_complete()
        ...
        print(f"Episode:200_f1_lstm_rew:{episode + 1}/{num_episodes}, Total Reward: {episode_reward},Max Rate:{max_coverage_rate*100:.2f}%,Coverage Rate: {coverage_rate * 100:.2f}%")
