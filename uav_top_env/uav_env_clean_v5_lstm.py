...
class UAVEnv(gym.Env):
    ...
    def __init__(self,
                 ...
                 priority_targets_ratio=0.5):  # 新增优先目标点比例参数
        ...
        # 优先目标点参数
        self.priority_targets_ratio = priority_targets_ratio
    ...
    def step(self, actions):
        ...
        # 检查结束条件
        coverage_rate, fully_connected, _, _, priority_coverage_rate = self.calculate_coverage_complete()
    ...
    def _compute_rewards(self, speed_violations=None):
        ...
        priority_target_reward = self._compute_priority_target_reward() # 新增优先目标点覆盖奖励
    ...
    def _compute_connectivity_reward_advanced(self):
        ...
        if nx.is_connected(G):
            connectivity_reward = 5.0  # 增加连通奖励
        else:
            connectivity_reward = -5.0  # 增加不连通惩罚
        ...
        if nx.is_connected(G):
            ...
            if actual_edges == min_edges:
                structure_reward = 3.0  # 增加最优结构奖励
            elif actual_edges <= min_edges * 1.5:
                structure_reward = 1.5  # 适度冗余
            else:
                structure_reward = -1.5  # 增加过度连接惩罚
            
        # 3. 添加边缘分布奖励
        edge_distribution_reward = 0.0
        if nx.is_connected(G) and len(active_agents) > 2:
            # 计算每个节点的度
            ...
            # 奖励均匀的连接分布
            edge_distribution_reward = 2.0 * np.exp(-2.0 * std_dev)

        return connectivity_reward + structure_reward + edge_distribution_reward
    ...
    def _compute_coverage_reward_advanced(self):
        ...
        # 2. 基础覆盖奖励 - 使用覆盖率的平方
        base_coverage_reward = (coverage_rate ** 2.0) * 8.0
        ...
        # 计算每个目标是否被覆盖
        covered_targets = np.zeros(len(target_positions), dtype=bool)
        for i, target in enumerate(target_positions):
            for j in self.active_agents:
                if np.linalg.norm(target - self.agent_pos[j]) <= self.coverage_radius:
                    covered_targets[i] = True
                    break
        
        # 分离优先目标和普通目标
        priority_indices = np.array(self.priority_target_indices)
        non_priority_indices = np.array([i for i in range(len(target_positions)) if i not in priority_indices])
        
        # 计算未覆盖目标的接近奖励
        approach_reward = 0.0
        
        # 优先处理未覆盖的优先目标
        uncovered_priority = priority_indices[~covered_targets[priority_indices]] if len(priority_indices) > 0 else []
        if len(uncovered_priority) > 0:
            # 计算每个未覆盖优先目标到最近UAV的距离
            uncovered_targets = target_positions[uncovered_priority]
            all_distances = np.linalg.norm(agent_positions[:, None, :] - uncovered_targets[None, :, :], axis=2)
            min_distances = np.min(all_distances, axis=0)
            
            # 接近奖励：距离越小，奖励越大
            approach_reward += np.sum(np.exp(-2.0 * min_distances)) * 2.0
        
        # 然后处理未覆盖的普通目标
        uncovered_normal = non_priority_indices[~covered_targets[non_priority_indices]] if len(non_priority_indices) > 0 else []
        if len(uncovered_normal) > 0:
            # 如果优先目标已经全部覆盖，增加普通目标的接近奖励
            uncovered_targets = target_positions[uncovered_normal]
            all_distances = np.linalg.norm(agent_positions[:, None, :] - uncovered_targets[None, :, :], axis=2)
            min_distances = np.min(all_distances, axis=0)
            
            # 接近奖励：距离越小，奖励越大
            normal_weight = 1.0
            if priority_coverage_rate >= 0.95:  # 优先目标几乎全部覆盖
                normal_weight = 2.0  # 增加普通目标的权重
                
            approach_reward += np.sum(np.exp(-2.0 * min_distances)) * normal_weight
        
        # 4. 覆盖进度奖励
        progress_reward = 0.0
        
        # 优先目标覆盖进度奖励
        priority_progress = priority_coverage_rate ** 2.0 * 5.0
        progress_reward += priority_progress
        
        # 全局覆盖进度奖励
        if priority_coverage_rate >= 0.9:  # 优先目标几乎全部覆盖
            # 增加普通目标的覆盖进度奖励
            normal_coverage = (coverage_rate * len(target_positions) - np.sum(covered_targets[priority_indices])) / max(1, len(non_priority_indices))
            normal_progress = normal_coverage ** 2.0 * 3.0
            progress_reward += normal_progress
        
        # 5. 总奖励
        total_reward = base_coverage_reward + approach_reward + progress_reward
        
        # 6. 应用活跃UAV数量的缩放
        k_1 = 1.0 + 0.5 * len(self.active_agents)  # 适度的UAV数量缩放
        
        return k_1 * total_reward
    ...
    def _compute_topology_adaptation_reward(self):
        ...
        if self.episode_plan.get('executed', False):
            coverage_rate, _, _, _, _ = self.calculate_coverage_complete()
    ...
    def _calculate_boundary_penalty(self, agent_idx):
        ...
        # 增强边界惩罚参数
        max_penalty = 150.0          # 增加惩罚上限
        boundary_limit = self.world_size
        safe_margin = 0.15           # 增加安全边距
        penalty_factor = 60.0        # 增加惩罚因子
        penalty_exponent = 4.0       # 增加指数斜率

        # 计算到边界的最小距离
        min_dist_to_boundary = min(
            boundary_limit - abs(agent_pos[0]),  # x方向
            boundary_limit - abs(agent_pos[1])   # y方向
        )
        
        # 如果接近边界
        if min_dist_to_boundary < safe_margin:
            # 计算边界惩罚（距离边界越近，惩罚越大）
            normalized_dist = (safe_margin - min_dist_to_boundary) / safe_margin
            penalty -= penalty_factor * (np.exp(penalty_exponent * normalized_dist) - 1.0)
            penalty = max(penalty, -max_penalty)
        
        # 如果超出边界（理论上不应该发生，但以防万一）
        if min_dist_to_boundary < 0:
            penalty -= max_penalty * 2.0
    ...
    def draw(self):
        ...
        # 显示优先目标点覆盖率
        priority_text = f"Priority Coverage: {priority_coverage_rate:.3f}"
        text_surface = font.render(priority_text, True, (255, 69, 0))
